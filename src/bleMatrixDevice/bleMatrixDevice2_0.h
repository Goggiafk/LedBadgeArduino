...
{
    // this class is a modernized HAL for cyberbadge 1.0 and 1.1

    // it handles ws2812 matrix and BLE UART interface
    public:
    ...
    void init(){
        ...
        char badge_id[23];
        snprintf(badge_id, 22, "Cyberbadge-%llX", ESP.getEfuseMac());  
    ...
    void loop(){
        if (SerialBT.available()) {
            String text = SerialBT.readStringUntil('\0');
            Serial.println(text);
            setText(0x00, 0, 0, text);
            // Serial.println(SerialBT.readStringUntil('\0'));
        }
